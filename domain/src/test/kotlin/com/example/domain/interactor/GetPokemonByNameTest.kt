...
class GetPokemonByNameTest {
    ...
    fun `The first value emitted should be is 'Loading'`() = runBlocking {
        Mockito.`when`(fakePokemonRepository.getPokemonByName(pokemon.name)).thenReturn(pokemon)
        val result = pokemonInteractor.getPokemonByName("Name").first()
    ...
    fun `The last value emitted should be is 'Success'`() = runBlocking {
        Mockito.`when`(fakePokemonRepository.getPokemonByName(pokemon.name)).thenReturn(pokemon)
        val result = pokemonInteractor.getPokemonByName("Name").last()
        assert(result is Result.Success && result.data == pokemon)
    }

    @Test
    fun `Expected catching IOException`() = runBlocking {
        Mockito.`when`(fakePokemonRepository.getPokemonByName(pokemon.name)).then {
            throw IOException()
        }
        val result = pokemonInteractor.getPokemonByName("Name").last()
